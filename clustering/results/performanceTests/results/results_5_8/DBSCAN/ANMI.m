...
function SSS=ANMI_analytical_11(dimension, factor, type, algorithm)

 display(strjoin({type, dimension, factor}, "   "));
 cluster_strings = {algorithm, "_", type, "_dim", dimension, "size", factor, "x_results.txt"};
 cluster_file = strjoin(cluster_strings, "");
 mem = dlmread (cluster_file);
 mem=mem'(:)';
 
 truth_strings = {algorithm, "_", type, "_dim", dimension, "size", factor, "x_truth.txt"};
 truth_file = strjoin(truth_strings, "");
 true_mem = dlmread (truth_file);
 true_mem=true_mem'(:)';
...
disp('display len mem:');
disp(length(true_mem));
disp(length(mem));
...
SSS=[NMI AMI AVI EMI];
save_strings = {"compare_", algorithm, "_", type, "_dim", dimension, "size", factor, "x_results.txt"};
save_file = strjoin(save_strings, "");
dlmwrite(save_file, SSS);
